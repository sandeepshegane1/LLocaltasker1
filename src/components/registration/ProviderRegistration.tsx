...
export function ProviderRegistration() {
    ...
    try {
      const userData = {
        ...
        ...(providerType === 'FARMER' && {
          ...
          pestControlMethod: data.pestControlMethod,
  ...
  const renderServiceSelection = () => {
    ...
    switch (providerType) {
      case 'SERVICE':
        ...
        break;
      case 'WORKER':
        serviceOptions = WORKER_CATEGORIES;
        title = 'work categories';
        break;
      case 'FARMER':
        serviceOptions = FARMERS_SERVICES;
  ...
  const renderFarmerSpecificFields = () => (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold mb-4">Farmer-specific Information</h2>
      ...
      <div>
        ...
        <input
          ...
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500"
        />
        {errors.cropType && <p className="text-red-500 text-xs">{errors.cropType.message as string}</p>}
      </div>

      <div>
        <label className="block text-sm font-medium text-gray-700">Harvest Season</label>
        <select
          {...register('harvestSeason', { required: 'Harvest season is required' })}
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500"
        >
        ...
        {errors.harvestSeason && <p className="text-red-500 text-xs">{errors.harvestSeason.message as string}</p>}
      </div>
      ...
      <div>
        <label className="flex items-center">
          <input
            ...
            type="checkbox"
            className="rounded border-gray-300 text-emerald-600 shadow-sm focus:border-emerald-500"
          />
          <span className="ml-2">Organic Certification</span>
        </label>
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700">Pest Control Method</label>
        <input
          {...register('pestControlMethod')}
          type="text"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500"
        />
      </div>
      ...
      <div>
        ...
        <select
          {...register('farmingMethod', { required: 'Farming method is required' })}
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500"
        >
        ...
        {errors.farmingMethod && <p className="text-red-500 text-xs">{errors.farmingMethod.message as string}</p>}
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white shadow-md rounded-lg">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        {step === 1 && renderFirstStep()}
        {step === 2 && renderServiceSelection()}
        {step === 3 && providerType === 'FARMER' && renderFarmerSpecificFields()}
        {((step === 3 && providerType !== 'FARMER') || step === 4) && renderLocationStep()}
        
        <button
          type="submit"
          className="w-full py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700"
        >
          {step === 1 ? 'Next' : step === 2 ? 'Next' : (step === 3 && providerType === 'FARMER') ? 'Next' : 'Complete Registration'}
        </button>
      </form>
    </div>
  );
